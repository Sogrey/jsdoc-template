...
// 输出目录
const OUTPUT_DIR = resolve(__dirname, '../Build/SDK-dev')
// SDK 模板目录
...
// 格式化 source map 文件（开发版保留格式化的 JSON 便于查看）
function formatSourceMaps() {
  try {
    ...
    console.warn('格式化 source map 时出现问题:', error.message)
  }
}
...
function generateTypeDefinitions() {
  try {
    // 使用 TypeScript 编译器生成类型定义
    execSync('npx tsc --project ./tsconfig.types.json --declaration --emitDeclarationOnly --outDir ./Build/SDK-dev', {
...
// 构建 SDK（开发模式）
async function buildSDKDev() {
  console.log('\n========================================')
  console.log('开始构建 SDK (开发模式)')
  console.log('========================================\n')
  ...
  try {
    ...
    // 调用 Vite 构建
    await build({
      configFile: resolve(__dirname, '../vite.config.sdk.dev.ts'),
      mode: 'development'
    })

    // 开发版格式化 source map（包含源代码内容，格式化便于查看）
    console.log('\n格式化 source map 文件...\n')
    formatSourceMaps()
